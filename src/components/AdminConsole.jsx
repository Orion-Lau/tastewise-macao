...
function AdminLogin({ onDone }) {
  ...
  const submit = async (event) => { event.preventDefault(); if (!apiConfig.configured) { onDone(); return; } if (!identifier || !password) { setError("请输入管理员账号和密码。"); return; } setLoading(true); setError(""); try { await api.adminLogin(identifier, password); onDone(); } catch (nextError) { setError(nextError.message); } finally { setLoading(false); } };
  return <main className="admin-login"><div className="admin-login-mark"><span>審</span><strong>澳味智译</strong><small>PLATFORM REVIEW</small></div><form onSubmit={submit}><UserRoundCog size={27} /><span>INTERNAL ACCESS</span><h1>平台审核后台</h1><p>审核新店、菜单更新与高风险字段，并保留完整操作记录。</p><label>管理员账号<input value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder="admin@tastewise.mo" /></label><label>密码<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" placeholder="••••••••" /></label>{error ? <div className="admin-error"><AlertCircle size={14} />{error}</div> : null}<button type="submit" disabled={loading}>{loading ? <Loader2 className="spin" /> : <LockKeyhole />}{apiConfig.configured ? "安全登录" : "进入示范审核台"}</button><small><ShieldCheck size={13} />正式环境应由服务端验证管理员角色与操作权限。</small></form></main>;
}
...
export default function AdminConsole() {
  ...
  const decide = async (review, decision) => { if (decision === "reject" && !reason.trim()) return; setLoading(true); try { if (apiConfig.configured) await api.decideReview(review.id, { decision, reason }); else await new Promise((resolve) => setTimeout(resolve, 350)); setReviews((old) => old.filter((item) => item.id !== review.id)); setAudit((old) => [{ id: uid(), actor: "current_admin", action: decision === "approve" ? "approve_review" : "reject_review", target: review.shop_name, created_at: "刚刚" }, ...old]); setRejecting(""); setReason(""); } catch (nextError) { setError(nextError.message); } finally { setLoading(false); } };

  return <main className="admin-shell"><header><div><span>審</span><strong>澳味智译</strong><small>平台审核</small></div><div className="admin-api"><i className={apiConfig.configured ? "live" : ""} />{apiConfig.configured ? "Production API" : "Demo mode"}</div><ThemeToggle className="admin-theme" /><button onClick={() => { api.logout("admin"); setAuthenticated(false); }} type="button"><LogOut size={15} />登出</button></header><aside><nav><button className={tab === "reviews" ? "active" : ""} onClick={() => setTab("reviews")} type="button"><ClipboardCheck />待审核<span>{reviews.length}</span></button><button className={tab === "feedback" ? "active" : ""} onClick={() => setTab("feedback")} type="button"><MessageSquareText />游客反馈<span>{feedback.length}</span></button><button className={tab === "audit" ? "active" : ""} onClick={() => setTab("audit")} type="button"><Clock3 />操作日志</button></nav><p><ShieldCheck size={14} />自动风险检查仅供参考，最终决定必须由人工管理员作出。</p></aside><section className="admin-main">{loading ? <div className="admin-state"><Loader2 className="spin" />正在读取平台资料…</div> : null}{error ? <div className="admin-state error"><AlertCircle />{error}</div> : null}
    {tab === "reviews" ? <><div className="admin-title"><div><span>REVIEW QUEUE</span><h1>待审核项目</h1><p>检查文字、价格、翻译和过敏原字段。</p></div><div><strong>{reviews.length}</strong><small>待审核</small></div><div className="risk"><strong>{totalFlags}</strong><small>风险提示</small></div></div>{reviews.length ? <div className="admin-review-list">{reviews.map((review) => <article key={review.id}><button className="admin-review-head" onClick={() => setExpanded(expanded === review.id ? "" : review.id)} type="button"><span className={review.type === "new_shop" ? "new" : "update"}>{review.type === "new_shop" ? "新店" : "更新"}</span><div><strong>{review.shop_name}</strong><small>{review.submitted_at || review.created_at} · {(review.items || []).length} 道菜</small></div><i>{(review.items || []).reduce((sum, item) => sum + riskFlags(item).length, 0)} 项风险</i><ChevronDown size={17} /></button>{expanded === review.id ? <div className="admin-review-body">{(review.items || []).map((item) => { const flags = riskFlags(item); return <div className={flags.length ? "flagged" : ""} key={item.id}><span>{item.name}</span><p>{item.description}</p><strong>MOP {item.price}</strong>{flags.length ? <ul>{flags.map((flag) => <li key={flag}><FileWarning size={13} />{flag}</li>)}</ul> : <small><BadgeCheck size={13} />自动检查未发现矛盾</small>}</div>; })}<footer>{rejecting === review.id ? <div className="reject-box"><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="填写退回原因，商户将看到这段内容。" /><button onClick={() => setRejecting("")} type="button">取消</button><button disabled={!reason.trim()} onClick={() => decide(review, "reject")} type="button">确认退回</button></div> : <><button className="reject" onClick={() => setRejecting(review.id)} type="button"><XCircle />退回修改</button><button className="approve" onClick={() => decide(review, "approve")} type="button"><CheckCircle2 />审核通过</button></>}</footer></div> : null}</article>)}</div> : <div className="admin-empty"><ClipboardCheck /><h2>暂时没有待审核项目</h2><p>商户提交新店或菜单更新后会出现在这里。</p></div>}</> : null}
    {tab === "feedback" ? <AdminTable title="游客反馈" rows={feedback} columns={["shop_name", "category", "helpful", "created_at"]} /> : null}
    {tab === "audit" ? <AdminTable title="操作与修改日志" rows={audit} columns={["created_at", "actor", "action", "target"]} /> : null}
  </section></main>;
}
...
function AdminTable({ title, rows, columns }) {
  ...
  const visible = needle
    ...
    : rows;
  return <div><div className="admin-title"><div><span>PLATFORM DATA</span><h1>{title}</h1><p>数据由后台 API 返回。</p></div><label><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索" /></label></div><div className="admin-table"><div>{columns.map((column) => <strong key={column}>{column}</strong>)}</div>{visible.length ? visible.map((row) => <div key={row.id}>{columns.map((column) => <span key={column}>{typeof row[column] === "boolean" ? row[column] ? "有帮助" : "没帮助" : row[column] ?? "—"}</span>)}</div>) : <p className="admin-table-empty">没有匹配的记录。</p>}</div></div>;
}
