...
const COPY = {
  zh: {
    portal: "商戶工作台", loginTitle: "管理你的多語菜單", loginSub: "校對菜品、確認食安資料，提交後發布到遊客端。",
    account: "手機號或電郵", password: "密碼", login: "登入工作台", demo: "進入示範工作台",
    ...
    workflow: "本次菜單更新", workflowSub: "按順序完成，提交後由平台審核高風險資料。", continue: "繼續處理",
    ...
    dishName: "菜名", description: "一句話說明", submitTitle: "提交審核與發布", submitSub: "平台會核對過敏原等高風險字段，不影響目前線上菜單。",
    ready: "可以提交", notReady: "仍有項目未完成", submit: "提交審核", submitting: "正在提交…", submitted: "已提交，平台審核中",
    onlineMenu: "目前線上菜單", qrHelp: "此為真實 QR Code，可下載後印在桌牌或收銀台。", downloadQr: "下載 QR Code", preview: "預覽遊客頁",
...
// 侧边导航按职能分组：菜单更新是有顺序的流程（与 Overview 步骤条一致，显示 01–05），
// 店铺经营与增长工具为平级功能。
const NAV_GROUPS = [
...
// 分类的存储值必须用 API 契约的稳定 id（与游客端 CATEGORIES 一致），中文只作显示。
...
function Login({ t, onDone }) {
  ...
  return (
    <main className="ms-login">
      <div className="ms-login-brand"><span>味</span><strong>澳味智译</strong><small>TASTEWISE MACAO · {t.portal}</small></div>
      <div className="ms-login-grid">
        ...
        <form className="ms-login-card" onSubmit={submit}>
          ...
          {apiConfig.configured ? <button className="ms-forgot" onClick={async () => { if (!identifier) { setError("请先输入手机号码或电邮。"); return; } setError(""); try { await api.requestPasswordReset(identifier); setResetMessage("如果账号存在，重设密码链接已经发送。"); } catch (nextError) { setError(nextError.message); } }} type="button">忘记密码？</button> : null}
...
export default function MerchantStudio({ lang, onPreview }) {
  ...
  const activateShop = (shop) => {
    ...
    // review_status 优先：审核中/被退回时商户端要看到流程状态，
    // 而游客端仍按 publication_status 展示线上菜单（版本隔离）
  ...
  return (
    <main className="ms-shell">
      <header className="ms-topbar">
        ...
        <div className="ms-work-brand"><span>味</span><div><strong>澳味智译</strong><small>{t.portal}</small></div></div>
      ...
      <section className="ms-main">
        ...
        {active === "safety" ? <SafetyPanel t={…2531 tokens truncated…d }) {
  ...
  return <div><PageHead kicker="CONTENT STUDIO" title={t.marketing} sub="营销 Agent 只能使用店铺档案和已发布商品，不会虚构折扣、奖项或成分。" />
    <section className="ms-panel ms-marketing"><div className="ms-platforms">{[["xiaohongshu", "小紅書"], ["instagram", "Instagram"], ["facebook", "Facebook"], ["wechat", "微信公眾號"]].map(([id, label]) => <button className={platform === id ? "active" : ""} onClick={() => setPlatform(id)} type="button" key={id}>{label}</button>)}</div><label>本次推广情境<textarea value={brief} onChange={(event) => setBrief(event.target.value)} placeholder="例如：中秋将至，主推杏仁饼礼盒；不可虚构折扣。" /></label><button className="ms-primary" onClick={generate} disabled={loading || !shopId} type="button">{loading ? <Loader2 className="spin" size={16} /> : <Megaphone size={16} />}{loading ? "正在生成…" : "生成平台文案"}</button>{error ? <p className="ms-error"><AlertCircle size={14} />{error}</p> : null}</section>
...
function InsightsPanel({ t, shopId, dashboard }) {
  ...
  useEffect(() => { if (!shopId) return; setLoading(true); setError(""); if (apiConfig.configured) api.getInsights(shopId, range).then(setData).catch((nextError) => setError(nextError.message)).finally(() => setLoading(false)); else { setTimeout(() => { setData({ total_questions: dashboard.questions || 26, staff_confirmation_rate: 19, categories: [{ key: "recommendation", label: "招牌推介", count: 11 }, { key: "payment", label: "支付方式", count: 6 }, { key: "dietary", label: "忌口需求", count: 5 }, { key: "allergen", label: "过敏原", count: 4 }], unanswered: 2 }); setLoading(false); }, 350); } }, [shopId, range]);
  const max = Math.max(...(data?.categories || []).map((row) => row.count), 1);
  return <div><PageHead kicker="ANONYMOUS INSIGHTS" title={t.insights} sub="只保存分类统计与反馈，不保存游客身份或完整对话原文。" action={<select className="ms-range" value={range} onChange={(event) => setRange(event.target.value)}><option value="7d">最近 7 天</option><option value="30d">最近 30 天</option><option value="90d">最近 90 天</option></select>} />
    {loading ? <div className="ms-data-state"><Loader2 className="spin" />正在读取洞察…</div> : error ? <div className="ms-data-state error"><AlertCircle />{error}</div> : data ? <><div className="ms-stat-grid"><Stat label="累计提问" value={data.total_questions || 0} tone="red" /><Stat label="需店员确认" value={`${data.staff_confirmation_rate || 0}%`} /><Stat label="未命中问题" value={data.unanswered || 0} /><Stat label="有帮助率" value={`${data.helpful_rate || 0}%`} tone="green" /></div><section className="ms-panel ms-insight-chart"><h2>提问类别分布</h2>{(data.categories || []).map((row) => <div key={row.key || row.label}><span>{row.label || row.key}</span><i><b style={{ width: `${Math.max(5, row.count / max * 100)}%` }} /></i><strong>{row.count}</strong></div>)}<p><ShieldCheck size={15} />“需店员确认”比例偏高时，建议补充成分资料或 FAQ，接待 Agent 才能可靠回答。</p></section></> : null}
  </div>;
}
...
function ImportPanel({ t, lang, shopName, setShopName, onItems }) {
  ...
  const submit = async () => { if (!file) { setError(t.emptyUpload); return; } setLoading(true); setError(""); try { let source; if (apiConfig.configured) { const result = await api.importMenu({ shopName, locale: lang, file }); source = result.items || result.dishes; if (!Array.isArray(source)) throw new Error("API 未返回 items 或 dishes 数组"); } else { await new Promise((r) => setTimeout(r, 900)); source = SAMPLE_SHOPS[0].dishes; } onItems(source.map((dish) => toItem(dish, false))); } catch (e) { setError(e.message); } finally { setLoading(false); } };
  return <div><PageHead kicker="MENU IMPORT" title={t.importTitle} sub={t.importSub} />
    <section className="ms-panel ms-import-panel"><label>{t.shopName}<input value={shopName} onChange={(e) => setShopName(e.target.value)} /></label><span className="ms-field-title">{t.file}</span>
      <div className={`ms-dropzone ${file ? "selected" : ""}`}><input ref={input} type="file" accept=".pdf,.jpg,.jpeg,.png,.heic,image/*,application/pdf" onChange={(e) => choose(e.target.files?.[0])} /><input ref={cameraInput} type="file" accept="image/*" capture="environment" onChange={(e) => choose(e.target.files?.[0])} />{file ? <FileCheck2 size={33} /> : <UploadCloud size={33} />}<strong>{file?.name || t.choose}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : t.formats}</small><div className="ms-upload-buttons"><button onClick={() => input.current?.click()} type="button">{t.choose}</button><button onClick={() => cameraInput.current?.click()} type="button"><Camera size={13} />拍照上传</button></div></div>
