...
本版本已完成 QwenPaw 基礎部署、場景拆分與智能體聯調。系統共設計 7 個職責獨立的智能體，其中接待問答、菜單翻譯、營銷文案、FAQ 生成及店鋪建檔已透過 QwenPaw 接入 `qwen3.7-plus`；菜單識別與風險檢查保留可審核的本地規則。所有模型輸出在進入業務流程前均經過字段校驗與安全後校驗。

已完成的線上聯調包括：

- 接待問答：只引用已發佈店鋪資料，忌口與高風險問題強制確認。
- 菜單翻譯：生成英、葡、日文草稿，保留人工校對流程。
- 店鋪建檔、FAQ 與營銷文案：均使用獨立 QwenPaw 智能體及結構化輸出契約。
- 安全回歸：9 項本地測試全部通過；平臺失敗時不會把未校驗內容寫入業務資料。

QwenPaw 保持運行時，可直接雙擊 `啟動.bat` 同時啟動前端與參考後端。模型 Token 由 QwenPaw 自身管理，專案與 GitHub 倉庫不保存 API Key。
